...
const metrics = [
  { 
    ...
    sublabel: "Python, Rust, TS, JS",
  },
  { 
    value: 3, 
    suffix: "rd Yr", 
    prefix: "",
    label: "B.Tech Undergrad Student",
  ...
  { 
    ...
    suffix: "", 
    prefix: "",
    label: "Major Hackathon Showcases",
    sublabel: "Stellar & InnovateX Concepts",
  },
];
...
export function MetricsSection() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Graph reveal
      gsap.fromTo(
        ".metrics-graph-reveal",
        { opacity: 0, scale: 0.98, filter: "blur(5px)" },
        {
          scrollTrigger: {
            trigger: ".metrics-graph-reveal",
            start: "top 85%",
            toggleActions: "play reverse play reverse",
          },
          opacity: 1,
      ...
      // Metrics cards reveals
      gsap.fromTo(
        ".metrics-card-reveal",
        { opacity: 0, y: 50, filter: "blur(4px)" },
        {
          scrollTrigger: {
            trigger: ".metrics-grid-container",
            start: "top 85%",
            toggleActions: "play reverse play reverse",
          },
          opacity: 1,
      ...
      // Ticker items reveal
      gsap.fromTo(
        ".metrics-ticker-reveal > *",
        { opacity: 0, x: -20 },
        {
          scrollTrigger: {
            trigger: ".metrics-ticker-reveal",
            start: "top 95%",
            toggleActions: "play reverse play reverse",
          },
          opacity: 1,
  ...
  return (
    <section ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        {/* Header */}
        <div className="grid lg:grid-cols-12 gap-8 mb-20 lg:mb-32 metrics-header">
          <div className="lg:col-span-8 lg:col-start-1">
            ...
            <h2 className="text-6xl md:text-7xl lg:text-[140px] font-display tracking-tight leading-[0.95]">
              Execution
              <br />
              <span className="text-muted-foreground">proficiency metrics.</span>
            </h2>
          </div>
        </div>

        {/* Organic graph image */}
        <div className="w-full mb-0 metrics-graph-reveal">
          <img
        ...
        {/* Metrics grid */}
        <div className="grid lg:grid-cols-3 gap-6 metrics-grid-container">
          {/* Large metric */}
          <div className="lg:col-span-1 bg-foreground/[0.02] border border-foreground/10 p-10 lg:p-14 metrics-card-reveal">
          ...
          {metrics.slice(1).map((metric, index) => (
            <div
              key={metric.label}
              className="bg-foreground/[0.02] border border-foreground/10 p-8 flex flex-col items-start justify-between gap-6 transition-all duration-700 metrics-card-reveal"
            >
        ...
        {/* Bottom ticker */}
        <div className="mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground metrics-ticker-reveal">
          <span>Soroban SDK</span>
          <span>Stellar Blockchain</span>
          <span>LangChain RAG</span>
          <span>React / Next.js</span>
          <span className="text-foreground">+ Python & Rust core stack</span>
        </div>
      </div>
    </section>
  );
